...
import {
  CommonResponseComponentProps,
  getClassName,
  getLabelPlacementStyle,
  getLocaleStringTextByCode,
  getStyleValueByKey,
} from "../../utils";
...
const secondsToInputTimeFormat = (
  ...
  const seconds = Math.floor(
    value - hourInSeconds * hours - minuteInSeconds * minutes
  );
...
const Time = forwardRef<InputHandleRef, TimeProps>((props, ref) => {
  const [response, setResponse] = useState<ResponseItem | undefined>(
    props.prefill
  );
  const [touched, setTouched] = useState(false);

  const [inputValue, setInputValue] = useState<undefined | number>(
    props.prefill && props.prefill.value
      ? parseFloat(props.prefill.value)
      : undefined
  );
  ...
  return (
    ...
    >
      ...
      <TimeInput
        id={props.parentKey}
        disabled={
          props.compDef.disabled !== undefined || props.disabled === true
        }
        min={minValue}
        max={maxValue}
      ...
      />
      {placeAfter ? (
        <label htmlFor={props.parentKey} className="ms-1">
          {getLocaleStringTextByCode(content, props.languageCode)}
        </label>
      ) : null}
    </div>
